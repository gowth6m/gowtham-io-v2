  ...
  return (
    <div className="fixed w-full origin-top">
      <nav className="mx-auto flex flex-row justify-between align-middle md:mt-8 md:px-10 mt-4 px-4">
        <a href="#home" className="text-sm z-40" aria-label="go to home section">
          <Logo color="var(--green-bright)" className="h-[40px] w-[40px] justify-start" />
        </a>
          ...
          {menuList.map((item) => (
            ...
            >
              <a href={item.href} aria-label="go to menu item">
                <span className="py-4 text-[var(--green-bright)]">{item.id}. </span> {item.name}
              </a>
            </li>
          ))}
        </ul>
      ...
      {menuOpened && (
        ...
        >
          {menuList.map((item) => (
            ...
            >
              <span className="text-[var(--green-bright)]">{item.id}. </span> {item.name}
            </motion.a>
          ))}
