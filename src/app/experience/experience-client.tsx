...
import { ExperienceModel, experience } from '@/components/experience/experience-data';
    ...
    >
      ...
      <div className="flex flex-col">
        {experience.map((entry: ExperienceModel, index: number) => (
          <ExperienceItem key={index} experience={entry} index={index} />
        ))}
      </div>
    </motion.div>
  );
  ...
  return (
    <motion.div variants={itemVariants} custom={index} className="overflow-hidden">
      <div className="grid grid-cols-3 md:grid-cols-5 my-2 gap-x-2 overflow-x-hidden overflow-hidden">
        <div className="md:col-span-1 hidden md:flex">{experience.dates}</div>
